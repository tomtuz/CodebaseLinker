...
export async function processCodebase(options: ProgramOptions): Promise<void> {
  ...
  const { options: structOptions, paths } = codebaseStruct;
  // const baseUrl = structOptions?.baseUrl ? path.resolve(basePath, paths) : process.cwd();
  const baseUrl = structOptions?.baseUrl ? path.resolve(basePath, structOptions.baseUrl) : process.cwd();
  // const baseUrl = structOptions?.baseUrl ?? process.cwd();
